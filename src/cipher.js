window.cipher = { 
  ...
  encode : (string , offset) => { 
    ...
    for (let i = 0; i < string.length; i++){
      ...
      const posicion_Ascii = string.toUpperCase().charCodeAt(i); 
      // el charCodeAt para obtener el codigo ascii de 1 letra
     
        if (posicion_Ascii === 32){
          resultado += " ";
        }else{
          const res_formula = (posicion_Ascii - 65 + (valorOffset)) %26 +65;
  ...
  decode : (string , offset) => { // metodo para cifrar
       ...
       for (let i = 0; i < string.length; i++){//lenght cuenta los caracteres del string
         const posicion_Ascii = string.toUpperCase().charCodeAt(i); // el charCodeAt para obtener el codigo ascii de 1 letra
         if (posicion_Ascii === 32){
          resultado += " ";
        }else{
        const res_formula = (posicion_Ascii + 65 - (valorOffset)) %26 +65;
